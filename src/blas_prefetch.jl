        ...
        if P_iter > M_iter + 1 # Excess of 1 is okay.
            ...
            q = quote
                    ...
                    for pmᵣ ∈ 1:$PM_ratio, pᵢ ∈ (pmᵣ-1)*$M_iter:$M_iter*pmᵣ - 1
                        for mᵢ ∈ $M_iter*pmᵣ - pᵢ:$(M_iter-2)
                            $(kernel)($D, $A, $X, Kernel{$L1M,$L1P,$stride_AD,$stride_X,$L1N}(),
                                $(prefetchA(L1M*T_size)))
                        end
                        if mᵢ ∈ $M_iter*pmᵣ - pᵢ < $M_iter
                            $(kernel)($pD + $((T_size*L1M)*(M_iter-1)) + $(T_size*L1P*stride_AD)*pᵢ,
                                $pA + $((T_size*L1M)*(M_iter-1)),
                                $X, Kernel{$L1M,$L1P,$stride_AD,$stride_X,$L1N}(),
                                $(prefetchA(L1M*(1-M_iter)*T_size)))
                        end
                        for mᵢ ∈ 0:$M_iter*pmᵣ - pᵢ - 2
                            $(kernel)($D, $A, $X, Kernel{$L1M,$L1P,$stride_AD,$stride_X,$L1N}(),
                                $(prefetchA(L1M*T_size)))
                        end
                        if $M_iter*pmᵣ - pᵢ > 0
                            $(kernel)($pD + $(T_size*L1M)*($M_iter*pmᵣ-pᵢ-1) + $(T_size*L1P*stride_AD)*pᵢ,
                                $pA + $(T_size*L1M)*($M_iter*pmᵣ-pᵢ-1),
                                $X, Kernel{$L1M,$L1P,$stride_AD,$stride_X,$L1N}(),
                                $(prefetchX(stride_X*L1P*T_size)))
                        end
                    end
                    for pᵢ ∈ $(M_iter*PM_ratio):$(P_iter-1)
                        for mᵢ ∈ $(M_iter*(PM_ratio+1))-pᵢ:$(M_iter-2)
                            $(kernel)($D, $A, $X, Kernel{$L1M,$L1P,$stride_AD,$stride_X,$L1N}(),
                                $(prefetchA(L1M*T_size)))
                        end
                        if $(M_iter*(PM_ratio+1))-pᵢ < $M_iter
                            $(kernel)($pD + $((T_size*L1M)*(M_iter-1)) + $(T_size*L1P*stride_AD)*pᵢ,
                                $pA + $((T_size*L1M)*(M_iter-1)),
                                $X, Kernel{$L1M,$L1P,$stride_AD,$stride_X,$L1N}(),
                                $(prefetchA(L1M*(1-M_iter)*T_size)))
                        end
                        for mᵢ ∈ 0:$(M_iter*(PM_ratio+1)-2)-pᵢ
                            $(kernel)($D, $A, $X, Kernel{$L1M,$L1P,$stride_AD,$stride_X,$L1N}(),
                                $(prefetchA(L1M*T_size)))
                        end
                        if mᵢ ∈ pᵢ < $(M_iter*(PM_ratio+1))
                            $(kernel)($pD + $(T_size*L1M)*($M_iter*pmᵣ-pᵢ-1) + $(T_size*L1P*stride_AD)*pᵢ,
                                $pA + $(T_size*L1M)*($M_iter*pmᵣ-pᵢ-1),
                                $X, Kernel{$L1M,$L1P,$stride_AD,$stride_X,$L1N}(),
                                $(prefetchX(stride_X*L1P*T_size)))
...
function cache_mulquote(M,N,P,stride_AD,stride_X,(L1M,L1N,L1P),(L2M,L2N,L2P),::Type{T}, init = :initkernel!, prefetchAX = ) where T
