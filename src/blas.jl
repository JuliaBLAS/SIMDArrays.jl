...
@generated function LinearAlgebra.mul!(D::SizedSIMDMatrix{M,P,T,ADR},
                            A::SizedSIMDMatrix{M,N,T,ADR},
                            X::SizedSIMDMatrix{N,P,T,XR}) where {M,N,P,T,ADR,XR}
...
D += A*X
"""
@generated function gemm!(D::SizedSIMDMatrix{M,P,T,ADR},
                            A::SizedSIMDMatrix{M,N,T,ADR},
                            X::SizedSIMDMatrix{N,P,T,XR}) where {M,N,P,T,ADR,XR}
...
@generated function gemm!(D::PtrMatrix{M,P,T,ADR},
    ...
    mulquote(M,N,P,ADR,XR,T,:kernel!)
end
...
function mulquote(M,N,P,ADR,XR,T,init=:initkernel!)
    ...
    elseif num == 1
        return cache_mulquote(M,N,P,ADR,XR,L1S,T,init)
    elseif num == 2
        return cache_mulquote(M,N,P,ADR,XR,L1S,L2S,T,init)
    else #num == 3
        # return cache_mulquote(ADR,N,P,ADR,XR,L1S,L2S,L3S,T,init)
        return cache_mulquote(M,N,P,ADR,XR,L1S,L3S,T,init) # should recurse calling mul and gemm!
    end
end
...
function cache_mulquote(M,N,P,stride_AD,stride_X,(L1M,L1N,L1P),::Type{T}, init = :initkernel!) where T
    ...
    q = quote
        pD, pA, pX = pointer(D), pointer(A), pointer(X)
        # @show Int(pD), Int(pA), Int(pX)
        # println("L1M: ", $L1M, "L1N: ", $L1N, "L1P: ", $L1P)
        # println("stride_AD: ", $stride_AD, "stride_X: ", $stride_X ,"T_size: ", $T_size,"initkernel: ", $init)
        # println("M_iter: ", $M_iter, "M_remain: ", $M_remain, "P_iter: ", $P_iter, "P_remain: ", $P_remain)
        # println("\n")
    ...
    # @show N, L1N
    # @show P, L1P
    # @show M_iter, M_remain
    # @show N_iter, N_remain
    # @show P_iter, P_remain
    # println("\n")
...
function cache_mulquote(M,N,P,stride_AD,stride_X,(L1M,L1N,L1P),(L2M,L2N,L2P),::Type{T}, init = :initkernel!) where T
    ...
    q = quote
        ...
        for pᵢ ∈ 0:$(P_iter-1)
            ...
            for mᵢ ∈ 0:$(M_iter-1)
                ...
                $initmul(pD_temp,
                     PtrMatrix{$L2M,$L2N,$T,$stride_AD}(pA + m_off),
                     PtrMatrix{$L2N,$L2P,$T,$stride_X}(pX + px_off))
                for nᵢ ∈ 1:$(N_iter-1)
                    gemm!(pD_temp,
                         PtrMatrix{$L2M,$L2N,$T,$stride_AD}(pA + m_off + nᵢ*$(L2N*stride_AD*T_size)),
                         PtrMatrix{$L2N,$L2P,$T,$stride_X}(pX + nᵢ*$(L2N*T_size) + px_off))
                end
                $(N_remain == 0 ? nothing : :(gemm!(pD_temp,
                     PtrMatrix{$L2M,$N_remain,$T,$stride_AD}(pA + m_off + $(N_iter*L2N*stride_AD*T_size)),
                     PtrMatrix{$N_remain,$L2P,$T,$stride_X}(pX + $(N_iter*L2N*T_size) + px_off))) )
            end
            ...
            $(M_remain == 0 ? nothing : quote
                ...
                $initmul(pD_temp,
                     PtrMatrix{$M_remain,$L2N,$T,$stride_AD}(pA + $(M_iter*L2M*T_size)),
                     PtrMatrix{$L2N,$L2P,$T,$stride_X}(pX + px_off))
                for nᵢ ∈ 1:$(N_iter-1)
                    gemm!(pD_temp,
                         PtrMatrix{$M_remain,$L2N,$T,$stride_AD}(pA + $(M_iter*L2M*T_size) + nᵢ*$(L2N*stride_AD*T_size)),
                         PtrMatrix{$L2N,$L2P,$T,$stride_X}(pX + nᵢ*$(L2N*T_size) + px_off))
                end
                $(N_remain == 0 ? nothing : :(gemm!(pD_temp,
                     PtrMatrix{$M_remain,$N_remain,$T,$stride_AD}(pA + $(M_iter*L2M*T_size + N_iter*L2N*stride_AD*T_size)),
                     PtrMatrix{$N_remain,$L2P,$T,$stride_X}(pX + $(N_iter*L2N*T_size) + px_off))) )
        ...
        push!(q.args,
            quote
                for mᵢ ∈ 0:$(M_iter-1)
                    ...
                    $initmul(pD_temp,
                         PtrMatrix{$L2M,$L2N,$T,$stride_AD}(pA + m_off),
                         PtrMatrix{$L2N,$P_remain,$T,$stride_X}(pX + $(P_iter*L2P*stride_X*T_size)))
                    for nᵢ ∈ 1:$(N_iter-1)
                        gemm!(pD_temp,
                             PtrMatrix{$L2M,$L2N,$T,$stride_AD}(pA + m_off + nᵢ*$(L2N*stride_AD*T_size)),
                             PtrMatrix{$L2N,$P_remain,$T,$stride_X}(pX + nᵢ*$(L2N*T_size) + $(P_iter*L2P*stride_X*T_size)))
                    end
                    $(N_remain == 0 ? nothing : :(gemm!(pD_temp,
                         PtrMatrix{$L2M,$N_remain,$T,$stride_AD}(pA + m_off + $(N_iter*L2N*stride_AD*T_size)),
                         PtrMatrix{$N_remain,$P_remain,$T,$stride_X}(pX + $(N_iter*L2N*T_size + P_iter*L2P*stride_X*T_size)))) )
                end
                ### Check if we need to add an expression for a remainder of M.
                $(M_remain == 0 ? nothing : quote
